...
void keyboard_handle_modifier(unsigned char scancode) {
    switch (scancode) {
        case 0x2A: case 0x36: shift_pressed = 1; break; // Left/Right Shift
        case 0xAA: case 0xB6: shift_pressed = 0; break; // Left/Right Shift release
        case 0x1D: ctrl_pressed = 1; break;             // Ctrl
...
unsigned char keyboard_get_scancode() {
    ...
    while (timeout--) {
        ...
        if (status & 1) {       // output buffer full
            if (status & 0x20)   // mouse data → skip
                continue;
...
unsigned char keyboard_poll_scancode(void) {
    uint8_t status = inb(KEYBOARD_STATUS_PORT);
    if (!(status & 1)) return 0;   // no data
    if (status & 0x20) return 0;   // mouse data → skip
    return inb(KEYBOARD_DATA_PORT);
}
