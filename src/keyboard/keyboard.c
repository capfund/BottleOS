...
void keyboard_handle_modifier(unsigned char scancode) {
    switch (scancode) {
        case 0x2A: case 0x36: shift_pressed = 1; break;
        case 0xAA: case 0xB6: shift_pressed = 0; break;
        case 0x1D: ctrl_pressed = 1; break;
        case 0x9D: ctrl_pressed = 0; break;
        case 0x3A: caps_lock_on ^= 1; break;
    }
}

char keyboard_scancode_to_ascii(unsigned char scancode) {
    static const char normal[128] = {0,27,'1','2','3','4','5','6','7','8','9','0','-','=','\b','\t',
        'q','w','e','r','t','y','u','i','o','p','[',']','\n',0,'a','s','d','f','g','h',
        'j','k','l',';','\'','`',0,'\\','z','x','c','v','b','n','m',',','.','/',0,'*',0,' ',0};
    static const char shifted[128] = {0,27,'!','@','#','$','%','^','&','*','(',')','_','+','\b','\t',
        'Q','W','E','R','T','Y','U','I','O','P','{','}','\n',0,'A','S','D','F','G','H',
        'J','K','L',':','"','~',0,'|','Z','X','C','V','B','N','M','<','>','?',0,'*',0,' ',0};
...
unsigned char keyboard_get_scancode(void) {
    // Drain any pending scancodes if needed
    uint8_t status = inb(KEYBOARD_STATUS_PORT);
