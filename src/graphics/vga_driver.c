...
static unsigned char g_320x200x256[] =
{
    ...
    0x41,0x00,0x0F,0x00,0x00 // AC
};

// ---------- Backbuffer ----------
static unsigned char backbuffer[320*200];

// ---------- VGA functions ----------
static void vga_init() {
    write_regs(g_320x200x256);
    memset(backbuffer, 0, sizeof(backbuffer));
    // copy backbuffer to VGA once
...
// Flush backbuffer to VGA memory (call once per frame)
...
// Optional: expose flush so main loop can draw frame once
void graphics_flush() {
    vga_flush();
}
