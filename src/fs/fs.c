...
int fs_init(void) {
    ...
    if (superblock.magic != FS_MAGIC) {
        ...
        */
        ...
        uint32_t file_table_blocks = (file_table_bytes + FS_BLOCK_SIZE - 1) / FS_BLOCK_SIZE;
        ...
        for (uint32_t i = 0; i < file_table_area; i++) p[i] = 0;
        /* zero data blocks */
        uint8_t *d = mod + superblock.data_block * FS_BLOCK_SIZE;
        uint32_t data_bytes = (total_blocks - superblock.data_block) * FS_BLOCK_SIZE;
        for (uint32_t i = 0; i < data_bytes; i++) d[i] = 0;
    } else {
    ...
    file_table = (fs_file_entry_t *)((uint8_t*)disk_module_addr + superblock.file_table_block * FS_BLOCK_SIZE);
    data_area = (uint8_t*)disk_module_addr + superblock.data_block * FS_BLOCK_SIZE;
...
static fs_file_entry_t* find_entry(const char *name) {
    for (uint32_t i = 0; i < superblock.max_files; i++) {
        if (file_table[i].used && k_strncmp(file_table[i].name, name, FS_FILENAME_LEN) == 0) {
            return &file_table[i];
        }
    }
    return NULL;
...
static int allocate_blocks(uint32_t blocks_needed) {
    /* build a simple bitmap of used blocks */
    //uint8_t *used = (uint8_t*)0;
    /* brute-force: check for a free run starting at each candidate */
...
int fs_create_file(const char *name) {
    if (find_entry(name)) return -2; /* already exists */
    for (uint32_t i = 0; i < superblock.max_files; i++) {
...
int fs_write_file(const char *name, const uint8_t *data, uint32_t size) {
    ...
    if (start < 0) return -2; /* no contiguous space */
    ...
    e->size = size;
    return fs_sync(); /* persist metadata+data in module memory */
}

int fs_read_file(const char *name, uint8_t *buf, uint32_t bufsize) {
    fs_file_entry_t *e = find_entry(name);
    if (!e) return -1;
    if (bufsize < e->size) return -2;
    if (e->start_block == 0xFFFFFFFF) return 0;
    uint8_t *src = data_area + e->start_block * FS_BLOCK_SIZE;
    for (uint32_t i = 0; i < e->size; i++) buf[i] = src[i];
    return (int)e->size;
}
...
int fs_delete_file(const char *name) {
    ...
    if (superblock.num_files > 0) superblock.num_files--;
    return fs_sync();
}
...
int fs_sync(void) {
    ...
    /* If you also want to persist file data: each file already written directly to data_area, which points into module memory */
